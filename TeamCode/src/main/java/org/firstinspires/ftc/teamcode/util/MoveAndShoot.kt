...
fun moveShootKinematics(relativeVector: Vector, fieldVelocity: Vector): Pair<Double, Double>{
    ...
    val heightDiff = 0.0 //TODO measure height difference from shooter and goal
    ...
    val phi = acos((-s_x+t*v_rx)/(v_s*cos(shooterAngle)))
    return Pair(v_s, phi)
}
...
fun calculatePredictiveMoveShoot(currentPose: Pose, fieldVelocity: Pose): Pair<Double, Double>?{
    ...
    return moveShootKinematics(closestIntersection, fieldVelocity.vector())
}