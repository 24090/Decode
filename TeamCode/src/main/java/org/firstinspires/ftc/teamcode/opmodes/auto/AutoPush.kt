...
open class AutoPush(val isRed: Boolean): LinearOpMode() {
    override fun runOpMode() {
        ...
        runBlocking(Race(
            ...
            Sequence(
                drive.goToCircle(Pose(144.0-robotLength/2, robotWidth/2, PI).mirroredIf(isRed)),
                Sleep(1.0),
                drive.goToCircle(Pose(144.0-robotLength/2, 48.0-robotWidth/2, PI).mirroredIf(isRed)),
                Sleep(1.0),
                drive.goToCircle(Pose(108.0, 60.0-robotWidth/2, PI).mirroredIf(isRed))
            ),
            Forever {
                drive.update()
            }
